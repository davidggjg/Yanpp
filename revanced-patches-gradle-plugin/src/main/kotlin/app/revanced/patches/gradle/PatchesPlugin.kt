...
abstract class PatchesPlugin : Plugin<Project> {
     ...
     *
     * Dependencies from the "patcherProvided" configuration (ReVanced Patcher and SMALI)
    ...
    private fun Project.configureJarTask(patchesExtension: PatchesExtension) {
        tasks.withType(Jar::class.java).configureEach {
            // Bundles built by this plugin contain patches compiled against
            // app.revanced.patcher, so they must be run by the ReVanced engine.
            // The extension is what tells Molt which engine a bundle needs:
            // .rvp -> ReVanced engine, .mpp -> Morphe engine.
            ...
            // the version transitively provided by revanced-patcher) don't cause provided
            ...
            it.manifest.apply {
                ...
                configurations
                    ...
                    .allDependencies
                    .firstOrNull { it.group == "app.revanced" && it.name == "revanced-patcher" }
                    ?.version?.let { version ->
